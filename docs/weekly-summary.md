...
6. 僅納入指定三位組員（蔡曜輿、張維良、阿汶），姓名依本機對照設定；多人卡片只列一次，並列符合名單的姓名。同工單號的不同卡片依最新去重規則合併。已封存卡片（closed=true）一律排除，包含已完成與值班事項；來源快照仍可保留封存資料供截止檢查，但不得納入週報。
7. 「Worktrack」標籤卡片獨立列於第四節，附目前階段，不在其他節重複；即使沒有指派人員也要列入，姓名欄寫「尚未安排」。其餘標籤視為系統名称；多標籤並列，空白標籤忽略，沒有系統標籤顯示未標示系統。Worktrack 卡片不受處理完畢清單缺少本期完成動作影響。無值班事項仍保留第四節並顯示「• 無符合條件的事項。」。
