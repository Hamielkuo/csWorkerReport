...
同一卡片多位指定人員並列一次，未指派指定三人者排除。以卡片 ID 去重，不合併同單號的不同卡片。非「值班」標籤均為系統名稱；「值班」卡片只列第四節，保留狀態；無值班事項仍保留第四節並顯示「• 無符合條件的事項。」。已封存卡片（closed=true）一律排除，包含已完成與值班事項；來源快照仍可保留封存資料供截止檢查，但不得納入週報。
